...
def run_age_st(df):
    ...
    df_values = df_values.groupby('TYPE')['DAMAGE'].value_counts().to_frame(name = 'COUNT').reset_index()
...
def run_age_opstions_st(df, week):
    ...
    df_groups = df_values.groupby(week)[['TYPE','DAMAGE']].value_counts().to_frame(name = 'COUNT').reset_index()
